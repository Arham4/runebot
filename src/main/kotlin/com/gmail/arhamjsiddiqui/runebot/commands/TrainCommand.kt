...
class TrainCommand : CommandExecutor {
    ...
    fun onTrainCommand(textChannel: TextChannel, user: User, args: Array<String>) {
        ...
        withPlayer(user, textChannel) { player ->
            ...
            if (skillId != null) {
                val exp = ThreadLocalRandom.current().nextInt(15 * player.skills.levels[skillId], if (ThreadLocalRandom.current().nextInt(0, 100) == 1) Math.pow(15 * player.skills.levels[skillId].toDouble(), 1.5).toInt() else Math.pow(15 * player.skills.levels[skillId].toDouble(), 1.2).toInt())
